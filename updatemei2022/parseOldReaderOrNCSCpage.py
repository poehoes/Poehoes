...
htmlFile = 'oldreaderncscupdate20201104test.html'
...
online = True
urlbase = 'https://advisories.ncsc.nl/advisory?id=NCSC-2022-0'
startkwetsnumb = 359
numbofpages = 7
#numbofpages includes the startpage
totalkwetsnr = startkwetsnumb + numbofpages - 1
...
while file_exists:
    fileName =  baseName + date + "-" + str(appendnr) + ".xml"
    appendnr = appendnr + 1
    file_exists = exists(fileName)
...
if online:
    ...
    try:
        toParse = BeautifulSoup(open(htmlFile), "html.parser")
    except:
