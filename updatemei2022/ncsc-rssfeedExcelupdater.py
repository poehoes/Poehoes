...
#
#install modules in windows with py -3.9 -m pip install SomePackage
#if you have python 3.9.x of course
#
...
#use this script online with ncsc-feed or offline for rss importing xml/html export. Close the Excelsheet!!
online = False
ncscrssurl = 'https://advisories.ncsc.nl/rss/advisories'
#i used to give the filename myself, but this is used once a month or so, i switched to just the date and not the vulnerability-range
#xmlimport_filename = 'ncscexport20210607-0464-0464.xml'
baseName = 'ncscexport'
#when uncommenting $xmlimport_filename adjust also code below where file is imported!

#the excelsheet's filename
#ncsc_sheet_filename = "G:\\Shared drives\\Security and Compliance\\5. IB\\Beheer\\Periodic_controls\\NCSC\\ncscnew.xlsx"
ncsc_sheet_filename = 'ncscnew.xlsx'

# END OF SETTINGS

# No editing needed below this line (except de 2nd try)
# --------------------------------

rssData = {}

def parseRSS (rssData,  scraperss):
...
if online:
    try:
        ...
        rssData = parseRSS (rssData, scraperss)
    except:
        print ("Url {} not available!".format(ncscrssurl))
else:
    #from a file, enable if local feed
    try:
        # if xmlimport_filename is set above in the Settings next two lines should be commented
        date = time.strftime('%Y%m%d')        
        xmlimport_filename = baseName + date + '.xml'
        appendnr = 1
        file_exists = exists(xmlimport_filename)
        while file_exists:
            scraperss = feedparser.parse(xmlimport_filename)
            rssData = parseRSS(rssData,  scraperss)
            xmlimport_filename =  baseName + date + "-" + str(appendnr) + ".xml"
            appendnr = appendnr + 1
            file_exists = exists(xmlimport_filename)
    except:
...
try:
    ...
    if online:
        print("Vulnerabilities were found online in the NCSC-feed {}".format(ncscrssurl))
        time.sleep(5)
    else:
